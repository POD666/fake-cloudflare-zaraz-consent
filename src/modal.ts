import { ModalConfig, ConsentPreferences, Purpose } from './types.js';
...
export class ConsentModal {
    ...
    modal.innerHTML = `
      <div style="margin-bottom: 20px;">
        ...
        <p style="margin: 0; color: ${
          isDark ? '#cbd5e0' : '#4a5568'
        }; line-height: 1.5;">${this.config.description}</p>
      </div>
      ...
      <div style="display: flex; gap: 12px; flex-wrap: wrap; justify-content: flex-end;">
        <button id="reject-all" style="${this.getButtonStyles(
          isDark,
          'secondary'
        )}">${this.config.rejectAllText}</button>
        <button id="accept-all" style="${this.getButtonStyles(
          isDark,
          'primary'
        )}">${this.config.acceptAllText}</button>
        <button id="save-preferences" style="${this.getButtonStyles(
          isDark,
          'primary'
        )}">${this.config.saveText}</button>
      </div>
    `;
  ...
  private createPurposeElement(purpose: Purpose, isDark: boolean): string {
    ...
    return `
      <div style="
        ...
        ${
          isRequired
            ? `background: ${isDark ? '#2d3748' : '#f7fafc'}; border-color: ${
                isDark ? '#63b3ed' : '#3182ce'
              };`
            : ''
        }
      ">
        <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 8px;">
          ...
          <label style="display: flex; align-items: center; cursor: ${
            isRequired ? 'not-allowed' : 'pointer'
          };">
            <input
              type="checkbox"
  ...
  private addEventListeners(modal: HTMLElement): void {
    ...
    // Accept all button
    const acceptButton = modal.querySelector(
      '#accept-all'
    ) as HTMLButtonElement;
